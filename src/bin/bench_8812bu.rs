// RTL8812BU Performance Benchmark
//
// Standalone binary for measuring RX throughput, channel switching speed.
// The north star adapter — compare other adapters against this.
//
// Usage:
//   cargo run --bin bench_8812bu
//   cargo run --bin bench_8812bu -- --hop --dwell 500
//   cargo run --bin bench_8812bu -- --channel 36 --duration 10
...
fn parse_args() -> BenchConfig {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                eprintln!("RTL8812BU Benchmark (baseline)");
                eprintln!();
                eprintln!("Usage:");
                eprintln!("  bench_8812bu                         Park on ch6, measure 30s");
                eprintln!("  bench_8812bu --channel 36            Park on ch36 (5GHz)");
                eprintln!("  bench_8812bu --channel 1 --band 6g   Park on 6GHz ch1");
                eprintln!("  bench_8812bu --hop --dwell 500       Hop all bands, 500ms/ch");
                eprintln!("  bench_8812bu --hop --bands 6g        Hop 6GHz only");
                eprintln!("  bench_8812bu --duration 60           Run for 60 seconds");
                std::process::exit(0);
            }
            _ => {}
        }
...
fn main() {
    ...
    eprintln!("╔══════════════════════════════════════════╗");
    eprintln!("║  RTL8812BU Performance Bench (baseline)  ║");
    eprintln!("╚══════════════════════════════════════════╝\n");

    // Known RTL8812BU VID:PIDs
    let devices = [
        (0x0BDA, 0xB812, "Realtek RTL8812BU (generic)"),
        (0x2357, 0x0115, "TP-Link Archer T4U V3"),
        (0x0B05, 0x1841, "ASUS USB-AC53 Nano"),
        (0x0B05, 0x184C, "ASUS USB-AC55 B1"),
        (0x0846, 0x9052, "Netgear A6100"),
    ];
    let mut driver = None;

    for (vid, pid, name) in &devices {
        match wifikit::chips::rtl8812bu::Rtl8812bu::open_usb(*vid, *pid) {
            Ok((d, ep)) => {
    ...
    let mut driver = match driver {
        Some(d) => d,
        None => {
            eprintln!("ERROR: No RTL8812BU device found. Plug one in and retry.");
            std::process::exit(1);
        }
    };
...
fn run_park_bench(driver: &mut wifikit::chips::rtl8812bu::Rtl8812bu, ch: Channel, cfg: &BenchConfig) {
...
fn run_hop_bench(driver: &mut wifikit::chips::rtl8812bu::Rtl8812bu, channels: &[Channel], cfg: &BenchConfig) {
