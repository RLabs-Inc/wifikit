// RTL8812BU RSSI Diagnostic — Park on channel, measure raw RSSI
//
// Same test as test_rssi_mt7921 but for RTL8812BU.
//
// Usage:
//   cargo run --bin test_rssi_8812bu -- --channel 161 --bssid 7C:10:C9:03:10:E4

use std::time::{Duration, Instant};
use wifikit::core::chip::ChipDriver;
use wifikit::core::Channel;
...
fn main() {
    ...
    let band = if channel >= 36 { wifikit::core::Band::Band5g } else { wifikit::core::Band::Band2g };

    eprintln!("╔═══════════════════════════════════════════╗");
    eprintln!("║  RTL8812BU RSSI Diagnostic                ║");
    eprintln!("╚═══════════════════════════════════════════╝\n");
    ...
    let devices = [
        ...
        (0x0BDA, 0xB812, "RTL8812BU Generic"),
    ];
    let mut driver = None;
    ...
    let mut driver = match driver {
        Some(d) => d,
        None => { eprintln!("ERROR: No RTL8812BU found!"); std::process::exit(1); }
    };
    ...
    driver.set_channel(Channel::new(channel)).expect("channel failed");
    eprintln!("OK\n");
    ...
    // ── Report ──
    let beacon_count: usize = bssid_stats.values().map(|v| v.len()).sum();
    eprintln!("  ═══ Results ═══\n");
    eprintln!("  Total frames:  {}", total_frames);
    eprintln!("  Beacon/Probe:  {}", beacon_count);
    eprintln!();
